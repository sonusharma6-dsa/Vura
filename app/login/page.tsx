...
const AUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  "This account uses Google sign-in. Please continue with Google.":
    "This account was created with Google. Continue with Google sign-in.",
  OAuthSignin:
    "Google sign-in failed. Check your OAuth client credentials and redirect URI.",
  OAuthCallback:
    "Google sign-in failed during callback. Verify NEXTAUTH_URL and your Google OAuth redirect URI.",
  OAuthAccountNotLinked:
    "This account is already linked to another sign-in method.",
  AccessDenied: "Access to Google sign-in was denied.",
  Configuration:
    "Authentication is not configured correctly. Check GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and NEXTAUTH_SECRET.",
};
...
function LoginContent() {
  ...
  const [error, setError] = useState(
    authError ? AUTH_ERROR_MESSAGES[authError] || "Sign in failed." : "",
  );
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(
        "Google sign-in failed. Check the provider configuration and environment variables.",
      );
    }
  };
        ...
        >
          ...
          <div className="text-left">
            <div className="flex justify-between items-center ml-1">
              <label className="text-xs text-[var(--color-neon-muted)]">
                Password
              </label>
              <Link
                href="/forgot-password"
                className="text-xs text-[var(--color-neon-primary)] hover:underline mb-1"
              >
                Forgot password?
              </Link>
            </div>
    ...
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center glow-bg">
          <Loader2 className="w-8 h-8 animate-spin text-[var(--color-neon-primary)]" />
        </div>
      }
    >
      <LoginContent />
